...
impl RadrootsCliSandbox {
    ...
    pub fn runtime_store_records(&self) -> Vec<RuntimeStoreRecord> {
        ...
        let executor = SqlxSqliteExecutor::open(path).expect("open runtime store db");
...
pub fn assert_no_removed_command_reference(value: &Value, args: &[&str]) {
    ...
    for removed in [
        ...
        "radroots trade ls",
        "radroots trade history",
        "radroots trade watch",
        "radroots trade new",
        "radroots trade create",
        "radroots farm init",
        "radroots farm check",
        "radroots relay ls",
        "radroots product",
...
pub fn seed_orderable_listing(sandbox: &RadrootsCliSandbox, listing_addr: &str) -> String {
    ...
    let executor = SqlxSqliteExecutor::open(Path::new(db_path)).expect("open replica db");
    assert_eq!(
...
pub fn seed_market_refresh_provenance(sandbox: &RadrootsCliSandbox, relay_urls: &[&str]) {
    let executor = SqlxSqliteExecutor::open(sandbox.replica_store_path()).expect("open replica db");
    executor
        .exec(
