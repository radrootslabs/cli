...
fn run() -> Result<(), ()> {
    ...
    match arguments.next().as_deref() {
        ...
        Some("rshr-step-302-platform-probe") if arguments.next().is_none() => {
            rshr_202_step_302_platform::run().map_err(|_| ())
        }
        _ => Err(()),
    }
}

fn parse_gate_args(values: Vec<String>) -> Result<rshr_202_step_302_gate::Arguments, ()> {
    let mut step = None;
    ...
    Ok(rshr_202_step_302_gate::Arguments {
        step: step.ok_or(())?,
