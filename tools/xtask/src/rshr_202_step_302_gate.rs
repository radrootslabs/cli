...
const EXACT_SOURCES: &[(&str, &str)] = &[
    (
        "Cargo.lock",
        "e0d86121841778d6ebb802bbfb807e7f74490a03b2a9593b15a7ac39020300ac",
    ),
    (
        "Cargo.toml",
        "9ac785043c27dcd5580efd7a4b7bd360706501f11737ce73ad76d8ba99ad9d1e",
    ),
    (
        "flake.lock",
    ...
    (
        "flake.nix",
        "376f25cb79d6dfab7a250c44a550a56b136207cde4ef282a71321e0e076fcb63",
    ),
];
...
fn canonical(value: &Value) -> Result<Vec<u8>, String> {
    serde_json::to_vec(value).map_err(|_| "Step 302 JSON encoding failed".to_owned())
}
...
fn resolve_nix() -> Result<PathBuf, String> {
    if let Some(explicit) = env::var_os("RSHR_NIX_EXECUTABLE") {
        return fs::canonicalize(explicit)
            .map_err(|_| "Step 302 Nix client is unavailable".to_owned());
    }
    let path = env::var_os("PATH").ok_or_else(|| "Step 302 PATH is absent".to_owned())?;
    env::split_paths(&path)
        .map(|directory| directory.join("nix"))
        .find(|candidate| candidate.is_file())
        .and_then(|candidate| fs::canonicalize(candidate).ok())
        .ok_or_else(|| "Step 302 Nix client is unavailable".to_owned())
}

fn object_keys(value: &Value, label: &str) -> Result<Vec<String>, String> {
    let mut keys = value
        .as_object()
        .ok_or_else(|| format!("Step 302 {label} is not an object"))?
        .keys()
        .cloned()
        .collect::<Vec<_>>();
    keys.sort_unstable();
...
fn require_lock() -> Result<(), String> {
    let lock: Value = serde_json::from_slice(
        &fs::read(root().join("flake.lock"))
            .map_err(|_| "Step 302 flake lock is unreadable".to_owned())?,
    )
    .map_err(|_| "Step 302 flake lock is invalid".to_owned())?;
    ...
    {
        return Err("Step 302 exact Nix tooling input differs".to_owned());
    }
    Ok(())
}
...
fn require_outputs(nix: &Path) -> Result<(), String> {
    ...
    for system in systems {
        if object_keys(&inventory["apps"][system], "apps")? != ["default"]
            ...
            || inventory["apps"][system]["default"]["description"] != "Run the built radroots CLI"
            || inventory["checks"][system]["default"]["name"] != "radroots-cli-check-1"
            || inventory["packages"][system]["default"]["name"] != "radroots_cli-0.1.0"
        {
            return Err("Step 302 CLI output inventory differs".to_owned());
        }
    }
    for system in ["x86_64-darwin", "aarch64-linux", "x86_64-windows"] {
        rejected(
            ...
            "Step 302 excluded-system evaluation",
        )?;
    }
    for attribute in [
        ...
        rejected(
            ...
            "Step 302 unowned output evaluation",
        )?;
    }
    Ok(())
}
...
fn require_nix() -> Result<(), String> {
    ...
    if sha256(&fs::read(&executable).map_err(|_| "Step 302 Nix client is unreadable")?)
        != NIX_SHA256
    {
        return Err("Step 302 Nix client identity differs".to_owned());
    }
    let version = bounded(
        Command::new(&executable).arg("--version"),
        "Step 302 Nix version",
    )?;
    if sha256(&version.stdout) != NIX_VERSION_SHA256 {
        return Err("Step 302 Nix version differs".to_owned());
    }
    bounded(
...
fn expected_contract(verifier_sha256: &str) -> Value {
    json!({
        "argv_template": [
            ...
            "-q", "-p", "radroots_cli_xtask", "--", "rshr-step-302-gate", "--step={step}",
        ...
        "assertion_id": [format!("step_302_gate_01_{GATE_DIGEST}")],
        ...
        "step": STEP,
        "verifier_path": "tools/xtask/src/rshr_202_step_302_gate.rs",
...
pub(crate) fn run(arguments: Arguments) -> Result<(), String> {
    ...
    {
        return Err("Step 302 gate arguments differ".to_owned());
    }
    let root = root();
    if root.join(".github").exists() || root.join(".act").exists() || root.join("docs").exists() {
        return Err("Step 302 forbidden repository root is present".to_owned());
    }
    for (relative, expected) in EXACT_SOURCES {
        let bytes = fs::read(root.join(relative))
            .map_err(|_| "Step 302 governed source is unreadable".to_owned())?;
        if sha256(&bytes) != *expected {
            return Err("Step 302 governed source bytes differ".to_owned());
        }
    }
    let flake_source = fs::read_to_string(root.join("flake.nix"))
        .map_err(|_| "Step 302 flake source is unreadable".to_owned())?;
    for forbidden in [
        ...
        if flake_source.contains(forbidden) {
            return Err("Step 302 checkout wrapper or unowned output is present".to_owned());
        }
    }

    let verifier_path = root.join("tools/xtask/src/rshr_202_step_302_gate.rs");
    let verifier_sha256 =
        sha256(&fs::read(verifier_path).map_err(|_| "Step 302 verifier is unreadable")?);
    let authority_path = root.join("contracts/rshr-202-step-302-gates.v1.json");
    let authority_bytes =
        fs::read(authority_path).map_err(|_| "Step 302 gate authority is unreadable")?;
    let authority: Value = serde_json::from_slice(&authority_bytes)
        .map_err(|_| "Step 302 gate authority is invalid".to_owned())?;
    ...
    let contracts = authority
        ...
        .ok_or_else(|| "Step 302 gate contract is absent".to_owned())?;
    if authority_bytes != canonical_authority
        || authority.get("schema")
            != Some(&Value::String(
                "radroots.cli.rshr-202-step-302-gates.v1".to_owned(),
            ))
        || authority.get("step") != Some(&json!([STEP]))
        || contracts.as_slice() != [expected_contract(&verifier_sha256)]
    {
        return Err("Step 302 gate authority differs".to_owned());
    }
    ...
    bounded(
        ...
        "Step 302 Cargo check",
    )?;
    require_nix()?;

    let contract = &contracts[0];
    let assertion = json!([{
        "id": format!("step_302_gate_01_{GATE_DIGEST}"),
        "result": "pass"
    }]);
    let result = json!({
    ...
    std::io::Write::write_all(&mut std::io::stdout().lock(), &bytes)
        .map_err(|_| "Step 302 result write failed".to_owned())
}
