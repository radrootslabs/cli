...
const REQUEST_PATH: &str = ".git/rshr-step-302-platform-request-sha256";
...
fn canonical(value: &Value) -> Result<Vec<u8>, String> {
    serde_json::to_vec(value).map_err(|_| "Step 302 platform JSON encoding failed".to_owned())
}
...
fn uname(flag: &str) -> Result<String, String> {
    let output = Command::new("/usr/bin/uname")
        .arg(flag)
        .output()
        .map_err(|_| "Step 302 platform probe could not start uname".to_owned())?;
    if !output.status.success() || !output.stderr.is_empty() {
        return Err("Step 302 platform probe uname failed".to_owned());
    }
    let value = std::str::from_utf8(&output.stdout)
        .map_err(|_| "Step 302 platform probe uname output is not UTF-8".to_owned())?
        .strip_suffix('\n')
        .ok_or_else(|| "Step 302 platform probe uname output differs".to_owned())?;
    if value.is_empty() || value.contains('\n') || value.contains('\r') {
        return Err("Step 302 platform probe uname output differs".to_owned());
    }
    Ok(value.to_owned())
}
...
pub(crate) fn run() -> Result<(), String> {
    let request_bytes = fs::read(root().join(REQUEST_PATH))
        .map_err(|_| "Step 302 platform execution request is unavailable".to_owned())?;
    let raw_request = std::str::from_utf8(&request_bytes)
        .map_err(|_| "Step 302 platform execution request is not UTF-8".to_owned())?;
    ...
    {
        return Err("Step 302 platform execution request differs".to_owned());
    }
    ...
    if kernel_name != "Darwin" || std::env::consts::ARCH != "aarch64" {
        return Err("Step 302 platform identity differs".to_owned());
    }
    ...
    std::io::Write::write_all(&mut std::io::stdout().lock(), &bytes)
        .map_err(|_| "Step 302 platform result write failed".to_owned())
}
